 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */const rR=Math.log(2);class lR{constructor(t){const i=c=>parseInt(Math.log(c)/rR,10),r=c=>parseInt(Array(c+1).join("1"),2);this.count=i(t+1),this.current_=this.count-1;const o=r(this.count);this.bits_=t+1&o}nextBitIsOne(){const t=!(this.bits_&1<<this.current_);return this.current_--,t}}const Do=function(s,t,i,r){s.sort(t);const o=function(_,g){const v=g-_;let E,C;if(v===0)return null;if(v===1)return E=s[_],C=i?i(E):E,new at(C,E.node,at.BLACK,null,null);{const M=parseInt(v/2,10)+_,U=o(_,M),G=o(M+1,g);return E=s[M],C=i?i(E):E,new at(C,E.node,at.BLACK,U,G)}},c=function(_){let g=null,v=null,E=s.length;const C=function(U,G){const W=E-U,Ue=E;E-=U;const Ye=o(W+1,Ue),Re=s[W],Ze=i?i(Re):Re;M(new at(Ze,Re.node,G,null,Ye))},M=function(U){g?(g.left=U,g=U):(v=U,g=U)};for(let U=0;U<_.count;++U){const G=_.nextBitIsOne(),W=Math.pow(2,_.count-(U+1));G?C(W,at.BLACK):(C(W,at.BLACK),C(W,at.RED))}return v},h=new lR(s.length),d=c(h);return new At(r||t,d)};/**
 * @license
 * Copyright 2017 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
